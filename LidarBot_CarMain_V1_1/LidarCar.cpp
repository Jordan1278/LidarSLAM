 ...
 * \param 
 * - 雷达扫描的地图处理及其显示  Map processing and display of radar scans
 */
void LidarCar::MapDisplay(void){

  //每帧数据长度/雷达旋转速度/数据帧数  Data length of each frame/radar rotation speed/number of data frames
  ...
  { 
    //Cortrol_flag 模式为迷宫时标志  Flag when mode is maze
    //当成功获取到一帧数据，标志置位  When a frame of data is successfully obtained, the flag is set
    if (showAngle >=  359){
       Cortrol_flag = true;
       count++;
    }
    ...
    // distance[showAngle]为雷达扫描的直线距离，将其通过cos和sin分解为x y方向上的距离  is the straight-line distance scanned by the radar, which is decomposed into the distance in the x and y directions through cos and sin.
    ...
    mapdata[i * 4 + 3] = 236;
	  ...
	  //迷宫有效数据获取  获取角度为180到360的数据 Maze valid data acquisition: Obtain data with angles from 180 to 360
